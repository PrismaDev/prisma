...
return!1}}),suggestionsView=new SuggestionsView,UpdatesView=DialogView.extend({templateId:"#updates-template",args:{updatesStr:updatesStringsModel,layoutStr:layoutStringsModel},initTwitter:function(){twttr.widgets.load()}}),updatesView=new UpdatesView;function overriddenGet(a,b){var c=serverDictionary.get(b);return Backbone.Model.prototype.get.call(a,c)}
function overriddenAdd(a,b,c,d,e){var g=[];d=serverDictionary.get(d);e||(e={});_.each(b,function(b){typeof b==a.model?a.add(b):(b=new a.model($.extend({},e,{id:b[d]},b)),g.push(b))});return Backbone.Collection.prototype.add.call(a,g,c)}
...
subjectList.get(a).get("Turmas");var b=[];_.each(a.models,function(a){b.push(a.formatData())});return b}}),faltacursarModel=new FaltacursarModel,SelectedModel=Backbone.Model.extend({options:"",viewEl:"#main-selected-div tbody",maxRows:12,nOptions:3,defaults:{addedSinceLastView:0},initialize:function(){this.options=[];for(var a=0;a<this.maxRows;a++)this.options[a]=[];for(a=0;a<this.maxRows;a++)for(var b=0;b<this.nOptions;b++)this.options[a][b]=null;this.on("change:addedSinceLastView",function(a){mainView.changeBadge(a.get("addedSinceLastView"))})},
isSelected:function(a,b){for(var c=0;c<this.maxRows;c++)for(var d=0;d<this.nOptions;d++)if(this.options[c][d]&&this.options[c][d].subjectCode==a&&(!b||b==this.options[c][d].classId))return!0;return!1},setFromServer:function(a){var b=this;_.each(a,function(a){var d=a[serverDictionary.get("CodigoDisciplina")],e=a[serverDictionary.get("FK_Turma")],g=a[serverDictionary.get("NoLinha")],f=a[serverDictionary.get("Opcao")],h=a[serverDictionary.get("Vagas")];a=a[serverDictionary.get("QtdNaFrente")];b.options[g][f]=
{subjectCode:d,classCode:subjectList.getClass(e).get("CodigoTurma"),classId:e,vacancies:h,rank:a}})},postAll:function(a){for(var b=0;b<this.maxRows;b++)for(var c=0;c<this.nOptions;c++)this.options[b][c]=a[b][c];a=[];for(var d=0,b=0;b<this.maxRows;b++)for(c=0;c<this.nOptions;c++)null!=this.options[b][c]&&(a[d++]=this.formatForPost(b,c));$.ajax({type:"POST",url:"/api/selecionada",data:"json="+JSON.stringify(a),success:function(a){console.log("POST // All  // Msg: "+a)}})},formatForPost:function(a,b){return{NoLinha:a,
Opcao:b,FK_Turma:this.options[a][b].classId}},swapContent:function(a,b,c,d){var e=this.options[a][b];this.options[a][b]=this.options[c][d];this.options[c][d]=e;console.log(a+", "+b+","+c+", "+d);console.log(this.options);e=[];null!=this.options[a][b]&&e.push(this.formatForPost(a,b));null!=this.options[c][d]&&e.push(this.formatForPost(c,d));console.log(e);$.ajax({type:"POST",url:"/api/selecionada",data:"json="+JSON.stringify(e),success:function(a){console.log("POST // Swap  // Msg: "+a)}})},getData:function(){return this.options},
changeViews:function(a,b,c){microhorarioClasseslistView.changeRow(a,b,c);faltacursarClasseslistView.changeRow(a,b,c);faltacursarView.markAsSelected(a,this.isSelected(a));a=faltacursarModel.get("Optativas").models;for(idx in a)faltacursarView.changeOptativaLabel(a[idx].get("CodigoOptativa"),a[idx].countChosen());selectedView.render();selectedController.runSimulation()},removeClass:function(a,b){for(var c=0;c<this.maxRows;c++)for(var d=0;d<this.nOptions;d++)if(this.options[c][d]&&this.options[c][d].subjectCode==
a&&this.options[c][d].classId==b)return this.options[c][d]=null,subjectList.getClass(b),$.ajax({type:"DELETE",url:"/api/selecionada",data:"json="+JSON.stringify([{FK_Turma:b}]),success:function(c){console.log("DELETE // Disciplina: "+a+" // Turma: "+b+" // Msg: "+c)}}),this.changeViews(a,b,!1),!0;return!1},addClassToPosition:function(a,b,c,d){var e=subjectList.getClass(b),g=this;$.ajax({type:"POST",url:"/api/selecionada",data:"json="+JSON.stringify([{FK_Turma:b,NoLinha:c,Opcao:d}]),success:function(f){console.log("POST // Disciplina: "+
a+" // Turma: "+b+" // Msg: "+f);f=JSON.parse(f);g.options[c][d]={subjectCode:a,classCode:e.get("CodigoTurma"),classId:b,vacancies:f[b].Vagas,rank:f[b].Rank};g.changeViews(a,b,!0)}})},addClass:function(a,b){for(var c=0;c<this.maxRows;c++)for(var d=0;d<this.nOptions;d++)if(null!=this.options[c][d]&&this.options[c][d].subjectCode==a&&d!=this.nOptions-1&&null==this.options[c][d+1])return this.addClassToPosition(a,b,c,d+1),!0;for(c=0;c<this.maxRows;c++)if(null==this.options[c][0])return this.addClassToPosition(a,
b,c,0),!0;for(c=0;c<this.maxRows;c++)for(d=0;d<this.nOptions;d++)if(null==this.options[c][d])return this.addClassToPosition(a,b,c,d),!0;return!1}}),selectedModel=new SelectedModel,HelperModel=Backbone.Model.extend({defaults:{active:!0,text:"",selector:""},deactivate:function(){$(this.get("selector")).data("tooltip",!1);this.set("active",!1);$.ajax({url:"/api/aviso",type:"DELETE",data:'json=["'+this.id+'"]',dataType:"json",success:function(a){}})}}),HelpersList=Backbone.Collection.extend({model:HelperModel,
deactivateFromServer:function(a){for(var b in a)this.get(a[b][serverDictionary.get("CodAviso")]).set("active",!1)}}),helpersList=new HelpersList;helpersList.add({id:"ableSubjectRow",text:mainHelpersStringsModel.get("ableSubjectRowText")});helpersList.add({id:"blockedSubjectRow",text:mainHelpersStringsModel.get("blockedSubjectRowText")});helpersList.add({id:"warningSubjectRow",text:mainHelpersStringsModel.get("warningSubjectRowText")});helpersList.add({id:"optativaRow",text:mainHelpersStringsModel.get("optativaRowText")});
helpersList.add({id:"selectedSubjectRow",text:mainHelpersStringsModel.get("selectedSubjectRowText")});helpersList.add({id:"ableClassRow",text:mainHelpersStringsModel.get("ableClassRowText")});helpersList.add({id:"chosenClassRow",text:mainHelpersStringsModel.get("chosenClassRowText")});helpersList.add({id:"warningClassRow",text:mainHelpersStringsModel.get("warningClassRowText")});helpersList.add({id:"blockedClassRow",text:mainHelpersStringsModel.get("blockedClassRowText")});
helpersList.add({id:"disabledClassRow",text:mainHelpersStringsModel.get("disabledClassRowText")});helpersList.add({id:"timeCell",text:mainHelpersStringsModel.get("timeCellText")});helpersList.add({id:"dayCell",text:mainHelpersStringsModel.get("dayCellText")});helpersList.add({id:"emptyCell",text:mainHelpersStringsModel.get("emptyCellText")});helpersList.add({id:"classCell",text:mainHelpersStringsModel.get("classCellText")});helpersList.add({id:"selectedRow",text:mainHelpersStringsModel.get("selectedRowText")});
helpersList.add({id:"selectedClass",text:mainHelpersStringsModel.get("selectedClassText")});$.fn.dataTableExt.oApi.fnAddTr=function(a,b,c){"undefined"==typeof c&&(c=!0);var d=b.getElementsByTagName("td");if(d.length!=a.aoColumns.length)alert("Warning: not adding new TR - columns and TD elements must match");else{for(var e=[],g=0;g<d.length;g++)e.push(d[g].innerHTML);d=this.oApi._fnAddData(a,e);b._DT_RowIndex=d;a.aoData[d].nTr=b;a.aiDisplay=a.aiDisplayMaster.slice();c&&this.oApi._fnReDraw(a)}};
$.fn.dataTableExt.oApi.fnAddTrAfter=function(a,b,c,d){"undefined"==typeof d&&(d=!0);var e=b.getElementsByTagName("td");if(e.length!=a.aoColumns.length)alert("Warning: not adding new TR - columns and TD elements must match");else{for(var g=[],f=0;f<e.length;f++)g.push(e[f].innerHTML);e=this.oApi._fnAddData(a,g);b._DT_RowIndex=c+1;a.aoData[e].nTr=b;b=a.aoData[e];a.aoData.splice(e,1);a.aoData.splice(c+1,0,b);for(f=c+2;f<a.aoData.length;f++)a.aoData[f].nTr._DT_RowIndex=f;a.aiDisplay=a.aiDisplayMaster.slice();
...
initTime:c,subjectCode:null});for(var d=2;d<this.ndays+2;d++)0!=this.ttmat[c][d].span&&(null!=this.ttmat[c][d].subjectCode?$(a[b++]).data({subjectCode:this.ttmat[c][d].subjectCode,day:null,initTime:null}):$(a[b++]).data({day:d,initTime:c,subjectCode:null}))}var a=this.$el.find("td, th"),e=this;_.each(a,function(a){e.bindCell(a)})},formatString:function(a,b,c,d,e){d||(d=-1);e||(e=-1);a=b.get("Horarios").models[a].get("Unidade");return c+" - "+b.get("CodigoTurma")+"<br/><span>"+subjectList.get(c).get("NomeDisciplina")+
"</span><br/><span>("+a+")</span> - <span>"+e+"/"+d+"</span>"},processArray:function(a){for(var b=[],c=this.startH;c<this.endH;c++){b[c]=[];for(var d=2;d<this.ndays+2;d++)b[c][d]={string:"",span:1,customClass:null,subjectCode:null}}for(c=0;c<a.length;c++)for(var d=subjectList.getClass(a[c].classId),e=d.get("Horarios").models,g=0;g<e.length;g++){var f=e[g].get("DiaSemana"),h=e[g].get("HoraInicial"),k=e[g].get("HoraFinal");if(f)for(b[h][f].string=this.formatString(g,d,a[c].subjectCode,a[c].vacancies,
a[c].rank),b[h][f].span=Number(k)-Number(h),b[h][f].customClass="ttclass"+a[c].cssClass,b[h][f].subjectCode=a[c].subjectCode,h+=1;h<k;h++)b[h][f].span=0}return b},buildTableBody:function(a){void 0==a&&(a=[]);this.ttmat=this.processArray(a);a=document.createElement("tbody");for(var b=this.startH;b<this.endH;b++){var c=document.createElement("tr"),d=document.createElement("td");$(d).addClass("text-top");$(d).addClass("timeCell");$(d).html(b+":00");$(c).append(d);for(d=2;d<this.ndays+2;d++)if(0!=this.ttmat[b][d].span){var e=
document.createElement("td"),g=document.createElement("div");g.innerHTML=this.ttmat[b][d].string;""==this.ttmat[b][d].string?$(e).addClass("emptyCell"):$(e).addClass("classCell");this.ttmat[b][d].customClass&&$(e).addClass(this.ttmat[b][d].customClass);e.rowSpan=this.ttmat[b][d].span;e.appendChild(g);c.appendChild(e)}$(a).append(c)}return $(a).html()},initHelpers:function(){helpersList.get("emptyCell").set("selector","#main-timetable-div td.emptyCell");helpersList.get("dayCell").set("selector","#main-timetable-div th.dayCell");
helpersList.get("classCell").set("selector","#main-timetable-div td.classCell");helpersList.get("timeCell").set("selector","#main-timetable-div td.timeCell")},bindHelpers:function(){helperView.create("emptyCell");helperView.create("dayCell");helperView.create("timeCell");helperView.create("classCell")},render:function(a){this.$el.html(this.template({timetableStr:timetableStringsModel}));this.$el.find("tbody").html(this.buildTableBody(a));this.bindCallbacks();this.bindHelpers();mainView.equalMainDivsHeight()}}),
timetableView=new TimetableView,FaltacursarView=Backbone.View.extend({template:"",templateRow:"",subjectDatatableView:"",closedImg:"Setinha Mini copy.PNG",openImg:"Setinha Mini.PNG",defaultImgPath:"img/",subjectTableWrapper:"",classesDiv:"",subjectTable:"",subjectDatatable:"",subjectTableFilter:"",subjectTableHeader:"",subjectTableBody:"",initialize:function(){this.template=_.template($("#faltacursar-template").html());this.templateRow=_.template($("#faltacursar-row-template").html());var a=this;
$(window).resize(function(){a.resize()})},cache:function(){this.subjectTableWrapper=$("#faltacursar-subject-table_wrapper");this.classesDiv=$("#faltacursar-classes-div");this.subjectTable=$("#faltacursar-subject-table");this.subjectTableFilter=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_filter");this.subjectTableHeader=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_scrollHead");this.subjectTableBody=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_scrollBody")},
events:{"click #faltacursar-subject-table tr":"clickOnRow","click #faltacursar-subject-table .ementaButton":"clickOnEmenta","click #close-classes-button":"clickOnCloseClasses"},handleOptativa:function(a){var b=$(a).attr("id"),b=faltacursarModel.getSubjectsOptativa(b),c=this.subjectDatatable.fnGetPosition($(a)[0]);if($(a).hasClass("openOptativa")){$(a).removeClass("openOptativa");$(a).find("td.imgCell img").attr("src",this.defaultImgPath+this.closedImg);for(var d=$(a),e=0;e<b.length;e++)$(d).next().hasClass("subjectSelected")&&
this.closeClassesDiv(),this.subjectDatatable.fnDeleteRow(c+1,null,!1),d=$(d).next();this.subjectDatatable.fnDraw()}else $(a).addClass("openOptativa"),$(a).find("td.imgCell img").attr("src",this.defaultImgPath+this.openImg),b.reverse(),this.addRowsToTable(b,c),$(a).next().addClass("extraBorder");this.calculateScrollTop(a)},clickOnCloseClasses:function(){$(this.subjectTable).find("tr.subjectSelected").removeClass("subjectSelected");this.closeClassesDiv()},changeOptativaLabel:function(a,b){var c=$("#"+
a).find(".selected-label");$(c).html(b+" "+subjectTableStringsModel.get("nOptativasChosenStr"));b?$(c).removeClass("hidden"):$(c).addClass("hidden")},markAsSelected:function(a,b){row=$("#"+a);b?$(row).find(".name .selected-label").removeClass("hidden"):$(row).find(".name .selected-label").addClass("hidden")},closeClassesDiv:function(){$(this.subjectTableWrapper).addClass("whole").removeClass("half");this.calculateTableScroll();$(this.classesDiv).addClass("hidden")},clickOnRow:function(a){a=$(a.target).parents("tr");
if($(a).hasClass("optativa"))return this.handleOptativa(a);$(a.find("td").first()).hasClass("dataTables_empty")||($(a).hasClass("subjectSelected")?($(a).removeClass("subjectSelected"),this.closeClassesDiv()):($(this.subjectTable).find("tr.subjectSelected").removeClass("subjectSelected"),$(a).addClass("subjectSelected"),$(this.subjectTableWrapper).addClass("half").removeClass("whole"),this.calculateTableScroll(),$(this.classesDiv).removeClass("hidden"),$(this.classesDiv).addClass("almostHalf"),a=$(a).attr("id"),
a=faltacursarModel.getSubjectClasses(a).sort(function(a,c){return a.code>c.code?1:a.code<c.code?-1:0}),faltacursarClasseslistView.render(a),faltacursarClasseslistView.resize()))},clickOnEmenta:function(a){a.stopPropagation()},resize:function(){this.subjectDatatable||(this.subjectDatatable=$("#faltacursar-subject-table").dataTable());this.subjectDatatable.fnAdjustColumnSizing(!1);this.calculateTableScroll()},calculateScrollTop:function(a){a=this.subjectDatatable.fnGetPosition($(a)[0]);var b=0;$(this.subjectTableBody).find("tr").slice(0,
a+1).each(function(a){b+=$(this).height()});$(this.subjectTableBody).scrollTop(b)},calculateTableScroll:function(){var a=$(this.subjectTableWrapper).height(),b=$(this.subjectTableFilter).outerHeight(!0)+$(this.subjectTableHeader).outerHeight(!0),c=$(this.subjectTableBody).outerHeight(!0)-$(this.subjectTableBody).height();$(this.subjectTableBody).height(a-b-c)},initHelpers:function(){helpersList.get("ableSubjectRow").set("selector","#faltacursar-subject-table tr:not(.optativa,.subjectWarning, \t\t\t.subjectBlocked, .subjectSelected)");
helpersList.get("optativaRow").set("selector","#faltacursar-subject-table tr.optativa");helpersList.get("blockedSubjectRow").set("selector","#faltacursar-subject-table tr.subjectBlocked:not(.subjectSelected)");helpersList.get("warningSubjectRow").set("selector","#faltacursar-subject-table tr.subjectWarning:not(.subjectSelected)")},bindHelpers:function(){helperView.create("ableSubjectRow");helperView.create("optativaRow");helperView.create("warningSubjectRow");helperView.create("blockedSubjectRow")},
initJS:function(){var a=this;this.subjectDatatable=$("#faltacursar-subject-table").dataTable({sDom:"ft",bPaginate:!1,bScrollCollapse:!0,sScrollY:"200px",bSort:!1,oLanguage:{sZeroRecords:subjectTableStringsModel.get("noResultsStr"),sSearch:faltacursarStringsModel.get("searchLabel")},fnDrawCallback:function(b){a.calculateTableScroll()}});$("#faltacursar-subject-table_wrapper").addClass("whole");this.initHelpers()},addRowsToTable:function(a,b){for(idx in a){var c=this.templateRow({subject:a[idx],subjectTableStr:subjectTableStringsModel});
void 0==b?this.subjectDatatable.fnAddTr($(c)[0],!1):this.subjectDatatable.fnAddTrAfter($(c)[0],b,!1)}this.subjectDatatable.fnAdjustColumnSizing(!0);this.bindHelpers()},render:function(){var a=faltacursarModel.getTableRows().sort(function(a,c){return a.term!=c.term?a.term-c.term:a.code-c.code});this.$el.html(this.template({subjectTableStr:subjectTableStringsModel,faltacursarStr:faltacursarStringsModel}));this.initJS();this.cache();this.addRowsToTable(a);faltacursarClasseslistView.setElement("#faltacursar-classes-table-div");
this.subjectDatatable.fnAdjustColumnSizing(!1);this.calculateTableScroll()}}),faltacursarView=new FaltacursarView,MicrohorarioView=Backbone.View.extend({template:"",waitingTemplate:"",noQueryTemplate:"",resultsDiv:"",noQueryState:"noQuery",queryState:"query",waitingState:"waiting",events:{"click #more-filters-button":"moreFilters","click #less-filters-button":"lessFilters","click #open-filters-button":"openFilters","click #close-filters-button":"closeFilters","submit #microhorario-form":"query","reset #microhorario-form":"clear"},
moreFilters:function(){$("#hidden-filters").removeClass("hidden");$("#less-filters-button").removeClass("hidden");$("#more-filters-button").addClass("hidden");microhorarioClasseslistView.resize()},lessFilters:function(){$("#hidden-filters").addClass("hidden");$("#less-filters-button").addClass("hidden");$("#more-filters-button").removeClass("hidden");microhorarioClasseslistView.resize()},openFilters:function(){$("#microhorario-filter").removeClass("hidden");$("#open-filters-button").addClass("hidden");
$("#close-filters-button").removeClass("hidden");microhorarioClasseslistView.resize()},closeFilters:function(){$("#microhorario-filter").addClass("hidden");$("#open-filters-button").removeClass("hidden");$("#close-filters-button").addClass("hidden");microhorarioClasseslistView.resize()},searchFor:function(a,b,c){this.clear();null==a&&(a="");null==b&&(b="");null==c&&(c="");$('input[name="HoraInicial"]').attr("value",b);$('select[name="DiaSemana"]').find('option[value="'+a+'"]').attr("selected",!0);
$('input[name="CodigoDisciplina"]').attr("value",c);mainRouter.navigate("microhorario",{trigger:!0});this.openFilters();this.query()},query:function(){this.changeState(this.waitingState);microhorarioController.fetchData(!0);return!1},clear:function(){$('input[type="text"]').attr("value","");$('select[name="DiaSemana"]').find("option").attr("selected",!1);$('select[name="DiaSemana"]').find('option[value=""]').attr("selected",!0);$('input[type="checked"]').attr("checked",!1);this.changeState(this.noQueryState)},
setFiltersState:function(){0!=$("#hiddenFilters").find('input:checked, input[type="text"][value!=""],\t\t\t\toption[value!=""]:selected').length?this.moreFilters():this.lessFilters()},initialize:function(){this.template=_.template($("#microhorario-template").html());this.waitingTemplate=_.template($("#microhorario-waiting-template").html());this.noQueryTemplate=_.template($("#microhorario-noquery-template").html())},changeState:function(a,b){void 0==typeof b&&(b=[]);a==this.queryState?(this.setFiltersState(),
microhorarioClasseslistView.render(b)):a==this.noQueryState?this.$resultsDiv.html(this.noQueryTemplate({str:microhorarioStringsModel})):this.$resultsDiv.html(this.waitingTemplate({str:microhorarioStringsModel}))},resize:function(){microhorarioClasseslistView.resize()},bindValidators:function(){$('input[name="Creditos"]').change(function(a){microhorarioValidator.intMask(a.target,3)});$('input[name="HoraInicial"]').change(function(a){microhorarioValidator.hourMask(a.target)});$('input[name="HoraFinal"]').change(function(a){microhorarioValidator.hourMask(a.target)})},
render:function(){this.$el.html(this.template({str:microhorarioStringsModel,timetableStr:timetableStringsModel}));this.$resultsDiv=$("#microhorario-results");microhorarioClasseslistView.setElement(this.$resultsDiv);this.bindValidators();this.changeState(this.noQueryState)}}),microhorarioView=new MicrohorarioView,SelectedView=Backbone.View.extend({templateRow:"",template:"",templateDraggable:"",initialize:function(){this.templateRow=_.template($("#selected-row-template").html());this.template=_.template($("#selected-template").html());
this.templateDraggable=_.template($("#selected-draggable-template").html());this.initHelpers()},initHelpers:function(){helpersList.get("selectedRow").set("selector","#main-selected-div tr");helpersList.get("selectedClass").set("selector","#main-selected-div td:has(div):not(.radio)");$(helpersList.get("selectedClass").get("selector")).live("mouseover",function(a){a.stopPropagation()})},changeInfo:function(a,b){var c=$("#info-container");c.find("#qtd-credits").html(a+" "+selectedStringsModel.get("qtdCreditsLabel"));
c.find("#qtd-classes").html(b+" "+selectedStringsModel.get("qtdClassesLabel"))},events:{"click button.close":"deleteClass",'change input[type="radio"]':"radioChange"},radioChange:function(a){a=$(a.target).parent();a=$(a).parent();a=$("#main-selected-div tbody tr").index(a);selectedController.runSimulation(a)},resize:function(){this.calculateHeightDivs()},equalDroppables:function(){var a=this.$el.find("tbody tr").first(),b=a.width(),c=0,a=a.find("td.class-droppable").each(function(a){c+=$(this).width()}),
b=Math.ceil(100*c/(3*b));a.css("width",b+"%")},deleteClass:function(a){var b=$(a.target).parent("div");a=$(b).find('input[type="hidden"][name="subjectCode"]').attr("value");b=$(b).find('input[type="hidden"][name="classCode"]').attr("value");selectedModel.removeClass(a,b);this.equalDroppables()},buildRow:function(a,b){for(var c=[],d=0;d<b.length;d++)b[d]&&(c[d]=this.templateDraggable(b[d]));return this.templateRow({index:a,options:b,template:c})},buildSelected:function(a){this.$el.html(this.template({selectedStr:selectedStringsModel}));
for(var b=this.$el.find("tbody"),c=0;c<a.length;c++)$(b).append(this.buildRow(c,a[c]));for(c=a.length;c<selectedModel.maxRows;c++)$(b).append(this.buildRow(c,[]))},calculateHeightDivs:function(){$("#table-container").height($("#main-selected-div").height()-$("#info-container").outerHeight(!0))},render:function(){this.buildSelected(selectedModel.getData());this.initJS()},sortableInit:function(){var a=this;this.$el.find("tbody.selected-sortable").sortable({helper:function(a,c){var d=c.children(),e=
c.clone();e.children().each(function(a){$(this).width(d.eq(a).width());$(this).height(d.eq(a).height())});return e},start:function(b,c){var d={};a.$el.find('input[type="radio"]',this).each(function(){$(this).is(":checked")&&(d[$(this).attr("name")]=$(this).val());$(document).data("radio_checked",d)})}}).bind("sortstop",function(a,c){var d=$(document).data("radio_checked");$.each(d,function(a,b){$('input[name="'+a+'"][value="'+b+'"]').prop("checked",!0)});selectedController.sortLines();selectedController.runSimulation()})},
draggableInit:function(){this.$el.find("div.class-draggable").draggable({revert:"invalid",zIndex:1E3})},droppableInit:function(){this.$el.find("td.class-droppable").droppable({drop:function(a,b){selectedController.swapPlaces($(this),$(b.draggable).parent());$(this).has("div.class-draggable")&&$(b.draggable).parent().append($(this).find("div.class-draggable"));$(b.draggable).css({left:"0px",right:"0px",top:"0px",bottom:"0px"});$(this).append($(b.draggable));selectedController.runSimulation()},accept:".class-draggable"})},
bindHelpers:function(){helperView.create("selectedRow");helperView.create("selectedClass")},initJS:function(){this.sortableInit();this.draggableInit();this.droppableInit();this.bindHelpers()}}),selectedView=new SelectedView,HelperView=Backbone.Model.extend({el:"body",curr:null,off:!0,formatTooltip:function(a){return"<span>"+helpersList.get(a).get("text")+'</span><br/>\t\t\t<button id="'+a+'tooltip" class="btn btn-primary btn-small">'+mainHelpersStringsModel.get("tooltipButtonLabel")+"</button>"},
create:function(a,b){var c=helpersList.get(a);this.off||c.get("active")&&$(c.get("selector")).qtip({content:{text:this.formatTooltip(a)},position:{my:"top center",at:"bottom center",viewport:$(window)},style:{classes:"qtip-bootstrap qtip-blue"},show:{solo:!0},hide:{delay:"800"},events:{show:function(a,b){a.stopPropagation()}}})}}),helperView=new HelperView,MainView=Backbone.View.extend({el:"",template:"",rendered:!1,tabs:{faltacursar:{li:"main-faltacursar-li",div:"main-faltacursar-div",href:"#faltacursar",
view:faltacursarView},microhorario:{li:"main-microhorario-li",div:"main-microhorario-div",href:"#microhorario",view:microhorarioView},selected:{li:"main-selected-li",div:"main-selected-div",href:"#selected",view:selectedView}},defaultTab:"faltacursar",timetableDiv:"",sidebarDiv:"",tabsNav:"",initialize:function(){this.template=_.template($("#main-template").html());var a=this;$(window).resize(function(){a.equalMainDivsHeight()})},cache:function(){this.timetableDiv=document.getElementById("main-timetable-div");
this.sidebarDiv=document.getElementById("main-sidebar-div");this.tabsNav=document.getElementById("main-tabs-nav")},equalMainDivsHeight:function(){var a=$(this.timetableDiv).height()-1;$(this.sidebarDiv).height(a);a-=$(this.tabsNav).outerHeight(!0);faltacursarView.$el.height(a);microhorarioView.$el.height(a);selectedView.$el.height(a)},setActiveTab:function(a){$("#main-tabs-nav li").removeClass("active");$("#main-tab-panes div").removeClass("active");$("#"+this.tabs[a].li).addClass("active");$("#"+
this.tabs[a].div).addClass("active");this.tabs[a].view.resize();"selected"==a&&selectedModel.set("addedSinceLastView",0)},changeBadge:function(a){a?($("#selected-badge").html(a),$("#selected-badge").removeClass("hidden")):$("#selected-badge").addClass("hidden")},renderSubviews:function(){timetableView.setElement("#main-timetable-div");timetableView.render();faltacursarView.setElement("#main-faltacursar-div");faltacursarView.render();microhorarioView.setElement("#main-microhorario-div");microhorarioView.render();
selectedView.setElement("#main-selected-div");selectedView.render()},render:function(){this.$el.html(this.template({tabs:this.tabs,mainStr:mainStringsModel}));this.renderSubviews();this.rendered=!0;this.cache()}}),mainView=new MainView,ClasseslistView=Backbone.View.extend({templateTable:"",templateRow:"",el:"",subjectInfo:"",classesDatatable:null,classesTableHead:"",classesTableBody:"",events:{"click .dataTables_scrollBody tr":"clickOnClass"},clickOnClass:function(a){a=$(a.target).parents("tr");if(!$(a).hasClass("subjectDisabled")&&
!$(a.find("td").first()).hasClass("dataTables_empty")){var b=$(a).find('input[type="hidden"][name="subjectCode"]').attr("value"),c=$(a).find('input[type="hidden"][name="classId"]').attr("value");$(a).hasClass("classChosen")?($(a).removeClass("classChosen"),selectedModel.removeClass(b,c),0<selectedModel.get("addedSinceLastView")&&selectedModel.set("addedSinceLastView",selectedModel.get("addedSinceLastView")-1)):($(a).addClass("classChosen"),selectedModel.addClass(b,c),selectedModel.set("addedSinceLastView",
selectedModel.get("addedSinceLastView")+1))}},changeRow:function(a,b,c){this.$el.find("tr").each(function(){var d=$(this).find('input[type="hidden"][name="subjectCode"]').attr("value"),e=$(this).find('input[type="hidden"][name="classId"]').attr("value");d==a&&e==b&&(c?$(this).addClass("classChosen"):$(this).removeClass("classChosen"))})},cache:function(){this.classesTableHead=this.$(".dataTables_scrollHead");this.classesTableBody=this.$(".dataTables_scrollBody")},resize:function(){this.classesDatatable&&
this.classesDatatable.fnDraw(!1)},calculateTableScroll:function(){},initialize:function(){this.templateTable=_.template($("#classeslist-template").html());this.templateRow=_.template($("#classeslist-row-template").html())},initHelpers:function(){helpersList.get("ableClassRow").set("selector",".dataTables_scrollBody tr:not(.subjectDisabled, .classChosen, \t\t\t.subjectBlocked, .subjectWarning)");helpersList.get("chosenClassRow").set("selector",".dataTables_scrollBody tr.classChosen");helpersList.get("warningClassRow").set("selector",
".dataTables_scrollBody tr.subjectWarning:not(.classChosen)");helpersList.get("blockedClassRow").set("selector",".dataTables_scrollBody tr.subjectBlocked:not(.classChosen)");helpersList.get("disabledClassRow").set("selector",".dataTables_scrollBody tr.subjectDisabled")},initJS:function(){var a=this;this.classesDatatable=this.$el.find("table").dataTable({sDom:this.options.sDom,bPaginate:!1,bScrollCollapse:!0,sScrollY:"20000px",bSort:!1,oLanguage:{sEmptyTable:classesTableStringsModel.get("emptyTableStr"),
sSearch:faltacursarStringsModel.get("searchLabel")},fnDrawCallback:function(b){a.calculateTableScroll()}});this.initHelpers()},bindHelpers:function(){helperView.create("ableClassRow");helperView.create("chosenClassRow");helperView.create("warningClassRow");helperView.create("blockedClassRow");helperView.create("disabledClassRow")},addRowsToTable:function(a){for(idx in a){var b=this.templateRow({classO:a[idx],classesTableStr:classesTableStringsModel,subjectInfo:this.subjectInfo});this.classesDatatable.fnAddTr($(b)[0],
!1)}this.classesDatatable.fnAdjustColumnSizing(!0);this.bindHelpers()},render:function(a){this.$el.html(this.templateTable({classesTableStr:classesTableStringsModel,subjectInfo:this.subjectInfo}));this.initJS();this.cache();this.addRowsToTable(a);this.calculateTableScroll()}}),MicrohorarioClasseslistView=ClasseslistView.extend({subjectInfo:!0,waitingData:!1,waitingTemplate:"",alertTemplate:"",endOfDataMsg:!1,initialize:function(){this.templateTable=_.template($("#classeslist-template").html());this.templateRow=
_.template($("#classeslist-row-template").html());this.waitingTemplate=_.template($("#microhorario-waiting-template").html());this.alertTemplate=_.template($("#microhorario-end-of-data-template").html())},events:{"click .dataTables_scrollBody tr":"clickOnClass","click .dataTables_scrollBody tr .ementaButton":"clickOnEmenta"},clickOnEmenta:function(a){a.stopPropagation()},addNextPage:function(a,b){$(this.classesTableBody).find("div").remove();if(a)$(this.classesTableBody).append(this.alertTemplate({str:microhorarioStringsModel})),
this.endOfDataMsg=!0;else{var c=$(this.classesTableBody).scrollTop();this.addRowsToTable(b);$(this.classesTableBody).scrollTop(c)}this.waitingData=!1},bindEndOfScroll:function(){var a=this;this.$el.find(".dataTables_scrollBody").scroll(function(){if($(this).scrollTop()&&$(this).scrollTop()+$(this).innerHeight()==$(this)[0].scrollHeight&&!a.waitingData){a.waitingData=!0;var b=document.createElement("div");$(b).html(a.waitingTemplate({str:microhorarioStringsModel}));$(a.classesTableBody).append(b);
microhorarioController.fetchData(!1)}})},calculateTableScroll:function(){var a=0;$("#microhorario-filter").hasClass("hidden")||(a+=$("#microhorario-filter").outerHeight(!0));var a=a+$("#microhorario-togglefilter").outerHeight(!0),b=$(this.el).outerHeight(!0)-$(this.el).height(),a=$(this.el).parent().height()-a-b;$(this.el).height(a);b=$(this.classesTableHead).outerHeight();$(this.classesTableBody).height(a-b);this.bindEndOfScroll()}}),microhorarioClasseslistView=new MicrohorarioClasseslistView({sDom:"t"}),
FaltacursarClasseslistView=ClasseslistView.extend({subjectInfo:!1,calculateTableScroll:function(){var a=this.$el.parent().height(),b=this.$el.parent().find("#close-classes-div").outerHeight(!0),c=this.$el.find(".dataTables_filter").outerHeight(!0)+$(this.classesTableHead).outerHeight(!0),d=$(this.classesTableBody).outerHeight(!0)-$(this.classesTableBody).height(),a=a-c-b-d;a<$(this.classesTableBody).height()&&$(this.classesTableBody).height(a)}}),faltacursarClasseslistView=new FaltacursarClasseslistView({sDom:"ft"});
...
function SelectedController(){this.runSimulation=function(a){var b=[];if(void 0==a||0>a)a=-1;var c=$("#main-selected-div tbody tr"),d=0,e=0;_.each(c,function(c){for(d>a&&$(c).find('input[type="radio"]').first().attr("checked",!0);"none"!=$(c).find('input[type="radio"]:checked').attr("value");){var f=$(c).find('input[type="radio"]:checked').parent("td").next();if(0!=$(f).find("div").length){var h=$(f).find('input[type="hidden"][name="classCode"]').attr("value"),k=$(f).find('input[type="hidden"][name="subjectCode"]').attr("value"),
z=$(f).find('input[type="hidden"][name="vacancies"]').attr("value"),A=$(f).find('input[type="hidden"][name="rank"]').attr("value"),m=subjectList.get(k).get("Turmas").get(h),w=subjectList.get(k).get("Creditos"),x=!0,s;for(s in b){m.get("Horarios");var p=subjectList.getClass(b[s].classId),l;if(!(l=k==b[s].subjectCode)){l=p;p=m.get("Horarios").models;l=l.get("Horarios").models;var t=!0,y=void 0;for(y in p){var q=p[y];if(q.get("DiaSemana")){for(var B in l){var n=l[B];if(n.get("DiaSemana")&&q.get("DiaSemana")==
n.get("DiaSemana")){var u,r,v;u=q.get("HoraInicial");r=n.get("HoraInicial");v=q.get("HoraFinal");n=n.get("HoraFinal");u>r&&(r=u,v=n);if(v>r){t=!1;break}}}if(!t)break}}l=!t}if(l){x=!1;break}}if(x&&30>=e+w){f="";for(m=0;m<selectedModel.maxRows;m++)if($(c).hasClass("row"+m)){f=m;break}e+=w;b.push({classId:h,subjectCode:k,cssClass:f,vacancies:z,rank:A});break}}$(f).next().find('input[type="radio"]').attr("checked",!0)}d++});timetableView.render(b);selectedView.changeInfo(e,b.length)};this.sortLines=function(){for(var a=
[],b=0;b<selectedModel.maxRows;b++)a[b]=[];var c=$("#main-selected-div tbody tr"),b=0;_.each(c,function(c){c=$(c).find(".class-droppable");var e=0;_.each(c,function(c){var d=a[b],h=e,k;0==$(c).find("div").length?k=null:(k=$(c).find('input[type="hidden"][name="classCode"]').attr("value"),c=$(c).find('input[type="hidden"][name="subjectCode"]').attr("value"),k={subjectCode:c,classId:k,classCode:subjectList.get(c).get("Turmas").get(k).get("CodigoTurma")});d[h]=k;e++});b++});selectedModel.postAll(a)};
this.swapPlaces=function(a,b){var c,d,e,g;d=$("#main-selected-div tbody tr");c=$(d).index($(a).parent());d=$(d).index($(b).parent());e=$($(a).parent().children(".class-droppable")).index(a);g=$($(b).parent().children(".class-droppable")).index(b);selectedModel.swapContent(c,e,d,g)}}
