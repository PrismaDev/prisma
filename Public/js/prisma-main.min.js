...
LayoutView=Backbone.View.extend({el:"body",template:"",contentView:null,loggedIn:!1,dialogDiv:"#dialog-div",events:{"click #open-suggestions-link":"openSuggestionDialog","click #open-faq-link":"openFAQDialog","click #open-tutorial-link":"openTutorialDialog","click ul div.dropdown-menu":"preventClose"},openSuggestionDialog:function(){suggestionsView.render();$(this.dialogDiv).modal("show")},openFAQDialog:function(){faqView.render();$(this.dialogDiv).modal("show")},openTutorialDialog:function(){tutorialView.render();
$(this.dialogDiv).modal("show")},preventClose:function(a){a.stopPropagation()},initialize:function(){this.template=_.template($("#layout-template").html())},initJS:function(){$(".dropdown-toggle").dropdown();fbApiController.loadApi()},setView:function(a){this.contentView=a},prepareDialogs:function(){this.loggedIn&&suggestionsView.setElement(this.dialogDiv);faqView.setElement(this.dialogDiv);tutorialView.setElement(this.dialogDiv)},render:function(){var a={layoutStr:layoutStringsModel,loggedIn:this.loggedIn};
this.loggedIn||(a={layoutStr:layoutStringsModel,loggedIn:this.loggedIn,loginStr:loginStringsModel});this.$el.html(this.template(a));this.prepareDialogs();this.initJS();this.contentView&&this.contentView.setElement("#content-div")}});
...
var fbApiController=new FbApiController,SuggestionsStringsModel=Backbone.Model.extend({defaults:{dialogTitle:"Sugest\u00f5es/Bugs/Coment\u00e1rios...",submitButtonLabel:"Enviar coment\u00e1rio",resetButtonLabel:"Limpar campo"}}),suggestionsStringsModel=new SuggestionsStringsModel,SuggestionsView=DialogView.extend({templateId:"#suggestions-template",args:{suggestionsStr:suggestionsStringsModel},suggestionsUrl:"/api/sugestao",events:{"submit #suggestions-form":"sendSuggestion","keypress textarea":"toggleButton"},
toggleButton:function(a){a=a.target;console.log(a);""==a.value.trim()?$('#suggestions-form input[type="submit"]').addClass("disabled"):$('#suggestions-form input[type="submit"]').removeClass("disabled")},sendSuggestion:function(){if($('#suggestions-form input[type="submit"]').hasClass("disabled"))return!1;$.ajax({url:this.suggestionsUrl,type:"POST",data:$("#suggestions-form").serialize(),success:function(){$("#dialogDiv").modal("hide")}});return!1}}),suggestionsView=new SuggestionsView;
function overriddenGet(a,b){var c=serverDictionary.get(b);return Backbone.Model.prototype.get.call(a,c)}function overriddenAdd(a,b,c,d,e){var f=[],d=serverDictionary.get(d);e||(e={});_.each(b,function(b){typeof b==a.model?a.add(b):(b=new a.model($.extend({},e,{id:b[d]},b)),f.push(b))});return Backbone.Collection.prototype.add.call(a,f,c)}
...
"#main-timetable-div td.classCell");helpersList.get("timeCell").set("selector","#main-timetable-div td.timeCell")},bindHelpers:function(){helperView.create("emptyCell");helperView.create("dayCell");helperView.create("timeCell");helperView.create("classCell")},render:function(a){this.$el.html(this.template({timetableStr:timetableStringsModel}));this.$el.find("tbody").html(this.buildTableBody(a));this.bindCallbacks();mainView.equalMainDivsHeight()}}),timetableView=new TimetableView,FaltacursarView=
Backbone.View.extend({template:"",templateRow:"",subjectDatatableView:"",closedImg:"Setinha Mini copy.PNG",openImg:"Setinha Mini.PNG",defaultImgPath:"img/",subjectTableWrapper:"",classesDiv:"",subjectTable:"",subjectDatatable:"",subjectTableFilter:"",subjectTableHeader:"",subjectTableBody:"",initialize:function(){this.template=_.template($("#faltacursar-template").html());this.templateRow=_.template($("#faltacursar-row-template").html());var a=this;$(window).resize(function(){a.resize()})},cache:function(){this.subjectTableWrapper=
$("#faltacursar-subject-table_wrapper");this.classesDiv=$("#faltacursar-classes-div");this.subjectTable=$("#faltacursar-subject-table");this.subjectTableFilter=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_filter");this.subjectTableHeader=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_scrollHead");this.subjectTableBody=$("#faltacursar-subject-table_wrapper\t\t\t\t\t .dataTables_scrollBody")},events:{"click #faltacursar-subject-table tr":"clickOnRow","click #faltacursar-subject-table .ementaButton":"clickOnEmenta",
"click #close-classes-button":"clickOnCloseClasses"},handleOptativa:function(a){var b=$(a).attr("id"),b=faltacursarModel.getSubjectsOptativa(b),c=this.subjectDatatable.fnGetPosition($(a)[0]);if($(a).hasClass("openOptativa")){$(a).removeClass("openOptativa");$(a).find("td.imgCell img").attr("src",this.defaultImgPath+this.closedImg);for(var d=$(a),e=0;e<b.length;e++)$(d).next().hasClass("subjectSelected")&&this.closeClassesDiv(),this.subjectDatatable.fnDeleteRow(c+1,null,!1),d=$(d).next();this.subjectDatatable.fnDraw()}else $(a).addClass("openOptativa"),
$(a).find("td.imgCell img").attr("src",this.defaultImgPath+this.openImg),b.reverse(),this.addRowsToTable(b,c),$(a).next().addClass("extraBorder");this.calculateScrollTop(a)},clickOnCloseClasses:function(){$(this.subjectTable).find("tr.subjectSelected").removeClass("subjectSelected");this.closeClassesDiv()},changeOptativaLabel:function(a,b){var c=$("#"+a).find(".selected-label");$(c).html(b+" "+subjectTableStringsModel.get("nOptativasChosenStr"));b?$(c).removeClass("hidden"):$(c).addClass("hidden")},
markAsSelected:function(a,b){row=$("#"+a);b?$(row).find(".name .selected-label").removeClass("hidden"):$(row).find(".name .selected-label").addClass("hidden")},closeClassesDiv:function(){$(this.subjectTableWrapper).addClass("whole").removeClass("half");this.calculateTableScroll();$(this.classesDiv).addClass("hidden")},clickOnRow:function(a){a=$(a.target).parents("tr");if($(a).hasClass("optativa"))return this.handleOptativa(a);$(a.find("td").first()).hasClass("dataTables_empty")||($(a).hasClass("subjectSelected")?
($(a).removeClass("subjectSelected"),this.closeClassesDiv()):($(this.subjectTable).find("tr.subjectSelected").removeClass("subjectSelected"),$(a).addClass("subjectSelected"),$(this.subjectTableWrapper).addClass("half").removeClass("whole"),this.calculateTableScroll(),$(this.classesDiv).removeClass("hidden"),$(this.classesDiv).addClass("almostHalf"),a=$(a).attr("id"),a=faltacursarModel.getSubjectClasses(a).sort(function(a,c){return a.code>c.code?1:a.code<c.code?-1:0}),faltacursarClasseslistView.render(a),
faltacursarClasseslistView.resize()))},clickOnEmenta:function(a){a.stopPropagation()},resize:function(){this.subjectDatatable||(this.subjectDatatable=$("#faltacursar-subject-table").dataTable());this.subjectDatatable.fnAdjustColumnSizing(!1);this.calculateTableScroll()},calculateScrollTop:function(a){var a=this.subjectDatatable.fnGetPosition($(a)[0]),b=0;$(this.subjectTableBody).find("tr").slice(0,a+1).each(function(){b+=$(this).height()});$(this.subjectTableBody).scrollTop(b)},calculateTableScroll:function(){var a=
$(this.subjectTableWrapper).height(),b=$(this.subjectTableFilter).outerHeight(!0)+$(this.subjectTableHeader).outerHeight(!0),c=$(this.subjectTableBody).outerHeight(!0)-$(this.subjectTableBody).height();$(this.subjectTableBody).height(a-b-c)},initHelpers:function(){helpersList.get("ableSubjectRow").set("selector","#faltacursar-subject-table tr:not(.optativa,.subjectWarning, \t\t\t.subjectBlocked, .subjectSelected)");helpersList.get("optativaRow").set("selector","#faltacursar-subject-table tr.optativa");
helpersList.get("blockedSubjectRow").set("selector","#faltacursar-subject-table tr.subjectBlocked:not(.subjectSelected)");helpersList.get("warningSubjectRow").set("selector","#faltacursar-subject-table tr.subjectWarning:not(.subjectSelected)")},bindHelpers:function(){helperView.create("ableSubjectRow");helperView.create("optativaRow");helperView.create("warningSubjectRow");helperView.create("blockedSubjectRow")},initJS:function(){var a=this;this.subjectDatatable=$("#faltacursar-subject-table").dataTable({sDom:"ft",
bPaginate:!1,bScrollCollapse:!0,sScrollY:"200px",bSort:!1,oLanguage:{sZeroRecords:subjectTableStringsModel.get("noResultsStr"),sSearch:faltacursarStringsModel.get("searchLabel")},fnDrawCallback:function(){a.calculateTableScroll()}});$("#faltacursar-subject-table_wrapper").addClass("whole")},addRowsToTable:function(a,b){for(idx in a){var c=this.templateRow({subject:a[idx],subjectTableStr:subjectTableStringsModel});void 0==b?this.subjectDatatable.fnAddTr($(c)[0],!1):this.subjectDatatable.fnAddTrAfter($(c)[0],
b,!1)}this.subjectDatatable.fnAdjustColumnSizing(!0);this.bindHelpers()},render:function(){var a=faltacursarModel.getTableRows().sort(function(a,c){return a.term!=c.term?a.term-c.term:a.code-c.code});this.$el.html(this.template({subjectTableStr:subjectTableStringsModel,faltacursarStr:faltacursarStringsModel}));this.initJS();this.cache();this.addRowsToTable(a);faltacursarClasseslistView.setElement("#faltacursar-classes-table-div");this.subjectDatatable.fnAdjustColumnSizing(!1);this.calculateTableScroll()}}),
faltacursarView=new FaltacursarView,MicrohorarioView=Backbone.View.extend({template:"",waitingTemplate:"",noQueryTemplate:"",resultsDiv:"",noQueryState:"noQuery",queryState:"query",waitingState:"waiting",events:{"click #more-filters-button":"moreFilters","click #less-filters-button":"lessFilters","click #open-filters-button":"openFilters","click #close-filters-button":"closeFilters","submit #microhorario-form":"query","reset #microhorario-form":"clear"},moreFilters:function(){$("#hidden-filters").removeClass("hidden");
...
initHelpers:function(){helpersList.get("selectedRow").set("selector","#main-selected-div tr");helpersList.get("selectedClass").set("selector","#main-selected-div td:has(div):not(.radio)");$(helpersList.get("selectedClass").get("selector")).live("mouseover",function(a){a.stopImmediatePropagation()})},changeInfo:function(a,b){var c=$("#info-container");c.find("#qtd-credits").html(a+" "+selectedStringsModel.get("qtdCreditsLabel"));c.find("#qtd-classes").html(b+" "+selectedStringsModel.get("qtdClassesLabel"))},
...
initJS:function(){this.sortableInit();this.draggableInit();this.droppableInit()}}),selectedView=new SelectedView,HelperView=Backbone.Model.extend({el:"body",curr:null,formatTooltip:function(a){return"<span>"+helpersList.get(a).get("text")+'</span><br/>\t\t\t<button id="'+a+'tooltip" class="btn btn-primary btn-small">'+mainHelpersStringsModel.get("tooltipButtonLabel")+"</button>"},create:function(a){var b=helpersList.get(a);this.curr&&($(this.curr).tooltip("destroy"),this.curr=b.get("selector"));b.get("active")&&
($(b.get("selector")).tooltip({html:!0,title:this.formatTooltip(a),delay:{show:"800",hide:"800"},placement:"bottom",container:"body"}),$("#"+a+"tooltip").live("click",function(){helpersList.get(a).deactivate()}))}}),helperView=new HelperView,MainView=Backbone.View.extend({el:"",template:"",rendered:!1,tabs:{faltacursar:{li:"main-faltacursar-li",div:"main-faltacursar-div",href:"#faltacursar",view:faltacursarView},microhorario:{li:"main-microhorario-li",div:"main-microhorario-div",href:"#microhorario",
view:microhorarioView},selected:{li:"main-selected-li",div:"main-selected-div",href:"#selected",view:selectedView}},defaultTab:"faltacursar",timetableDiv:"",sidebarDiv:"",tabsNav:"",initialize:function(){this.template=_.template($("#main-template").html());var a=this;$(window).resize(function(){a.equalMainDivsHeight()})},cache:function(){this.timetableDiv=document.getElementById("main-timetable-div");this.sidebarDiv=document.getElementById("main-sidebar-div");this.tabsNav=document.getElementById("main-tabs-nav")},
equalMainDivsHeight:function(){var a=$(this.timetableDiv).height()-1;$(this.sidebarDiv).height(a);a-=$(this.tabsNav).outerHeight(!0);faltacursarView.$el.height(a);microhorarioView.$el.height(a);selectedView.$el.height(a)},setActiveTab:function(a){$("#main-tabs-nav li").removeClass("active");$("#main-tab-panes div").removeClass("active");$("#"+this.tabs[a].li).addClass("active");$("#"+this.tabs[a].div).addClass("active");this.tabs[a].view.resize();"selected"==a&&selectedModel.set("addedSinceLastView",
0)},changeBadge:function(a){a?($("#selected-badge").html(a),$("#selected-badge").removeClass("hidden")):$("#selected-badge").addClass("hidden")},renderSubviews:function(){timetableView.setElement("#main-timetable-div");timetableView.render();faltacursarView.setElement("#main-faltacursar-div");faltacursarView.render();microhorarioView.setElement("#main-microhorario-div");microhorarioView.render();selectedView.setElement("#main-selected-div");selectedView.render()},render:function(){this.$el.html(this.template({tabs:this.tabs,
mainStr:mainStringsModel}));this.renderSubviews();this.rendered=!0;this.cache()}}),mainView=new MainView,ClasseslistView=Backbone.View.extend({templateTable:"",templateRow:"",el:"",subjectInfo:"",classesDatatable:null,classesTableHead:"",classesTableBody:"",events:{"click .dataTables_scrollBody tr":"clickOnClass"},clickOnClass:function(a){a=$(a.target).parents("tr");if(!$(a).hasClass("subjectDisabled")&&!$(a.find("td").first()).hasClass("dataTables_empty")){var b=$(a).find('input[type="hidden"][name="subjectCode"]').attr("value"),
c=$(a).find('input[type="hidden"][name="classId"]').attr("value");$(a).hasClass("classChosen")?($(a).removeClass("classChosen"),selectedModel.removeClass(b,c),0<selectedModel.get("addedSinceLastView")&&selectedModel.set("addedSinceLastView",selectedModel.get("addedSinceLastView")-1)):($(a).addClass("classChosen"),selectedModel.addClass(b,c),selectedModel.set("addedSinceLastView",selectedModel.get("addedSinceLastView")+1))}},changeRow:function(a,b,c){this.$el.find("tr").each(function(){var d=$(this).find('input[type="hidden"][name="subjectCode"]').attr("value"),
...
".dataTables_scrollBody tr.subjectDisabled")},initJS:function(){var a=this;this.classesDatatable=this.$el.find("table").dataTable({sDom:this.options.sDom,bPaginate:!1,bScrollCollapse:!0,sScrollY:"20000px",bSort:!1,oLanguage:{sEmptyTable:classesTableStringsModel.get("emptyTableStr"),sSearch:faltacursarStringsModel.get("searchLabel")},fnDrawCallback:function(){a.calculateTableScroll()}})},bindHelpers:function(){helperView.create("ableClassRow");helperView.create("chosenClassRow");helperView.create("warningClassRow");
helperView.create("blockedClassRow");helperView.create("disabledClassRow")},addRowsToTable:function(a){for(idx in a){var b=this.templateRow({classO:a[idx],classesTableStr:classesTableStringsModel,subjectInfo:this.subjectInfo});this.classesDatatable.fnAddTr($(b)[0],!1)}this.classesDatatable.fnAdjustColumnSizing(!0);this.bindHelpers()},render:function(a){this.$el.html(this.templateTable({classesTableStr:classesTableStringsModel,subjectInfo:this.subjectInfo}));this.initJS();this.cache();this.addRowsToTable(a);
this.calculateTableScroll()}}),MicrohorarioClasseslistView=ClasseslistView.extend({subjectInfo:!0,waitingData:!1,waitingTemplate:"",alertTemplate:"",endOfDataMsg:!1,initialize:function(){this.templateTable=_.template($("#classeslist-template").html());this.templateRow=_.template($("#classeslist-row-template").html());this.waitingTemplate=_.template($("#microhorario-waiting-template").html());this.alertTemplate=_.template($("#microhorario-end-of-data-template").html())},events:{"click .dataTables_scrollBody tr":"clickOnClass",
"click .dataTables_scrollBody tr .ementaButton":"clickOnEmenta"},clickOnEmenta:function(a){a.stopPropagation()},addNextPage:function(a,b){$(this.classesTableBody).find("div").remove();if(a)$(this.classesTableBody).append(this.alertTemplate({str:microhorarioStringsModel})),this.endOfDataMsg=!0;else{var c=$(this.classesTableBody).scrollTop();this.addRowsToTable(b);$(this.classesTableBody).scrollTop(c)}this.waitingData=!1},bindEndOfScroll:function(){var a=this;this.$el.find(".dataTables_scrollBody").scroll(function(){if($(this).scrollTop()&&
$(this).scrollTop()+$(this).innerHeight()==$(this)[0].scrollHeight&&!a.waitingData){a.waitingData=!0;var b=document.createElement("div");$(b).html(a.waitingTemplate({str:microhorarioStringsModel}));$(a.classesTableBody).append(b);microhorarioController.fetchData(!1)}})},calculateTableScroll:function(){var a=0;$("#microhorario-filter").hasClass("hidden")||(a+=$("#microhorario-filter").outerHeight(!0));var a=a+$("#microhorario-togglefilter").outerHeight(!0),b=$(this.el).outerHeight(!0)-$(this.el).height(),
a=$(this.el).parent().height()-a-b;$(this.el).height(a);b=$(this.classesTableHead).outerHeight();$(this.classesTableBody).height(a-b);this.bindEndOfScroll()}}),microhorarioClasseslistView=new MicrohorarioClasseslistView({sDom:"t"}),FaltacursarClasseslistView=ClasseslistView.extend({subjectInfo:!1,calculateTableScroll:function(){var a=this.$el.parent().height(),b=this.$el.parent().find("#close-classes-div").outerHeight(!0),c=this.$el.find(".dataTables_filter").outerHeight(!0)+$(this.classesTableHead).outerHeight(!0),
d=$(this.classesTableBody).outerHeight(!0)-$(this.classesTableBody).height(),a=a-c-b-d;a<$(this.classesTableBody).height()&&$(this.classesTableBody).height(a)}}),faltacursarClasseslistView=new FaltacursarClasseslistView({sDom:"ft"});
...
helpersList.deactivateFromServer(DATA_VIEW.Data[serverDictionary.get("Avisos")]);layoutView.contentView=mainView;layoutView.loggedIn=!0;layoutView.render();mainView.render();selectedController.runSimulation()}}),mainRouter=new MainRouter;mainRouter.on("route:main",function(){this.loadPage();mainRouter.navigate(mainView.defaultTab,{trigger:!0})});
mainRouter.on("route:tabs",function(a){if(!mainView.tabs[a])return mainRouter.navigate(mainView.defaultTab,{trigger:!0,replace:!0});mainView.rendered||this.loadPage();mainView.setActiveTab(a)});Backbone.history.start();
