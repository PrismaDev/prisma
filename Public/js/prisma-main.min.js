...
waitingImgURL:"http://i.stack.imgur.com/FhHRx.gif",resizeW:function(){microhorarioClasseslistView.resizeW()},resizeH:function(){microhorarioClasseslistView.resizeH()},events:{"click #moreFiltersButton":"moreFilters","click #lessFiltersButton":"lessFilters","click #openFiltersButton":"openFilters","click #closeFiltersButton":"closeFilters"},moreFilters:function(){$("#hiddenFilters").removeClass("hidden");$("#lessFiltersButton").removeClass("hidden");$("#moreFiltersButton").addClass("hidden")},lessFilters:function(){$("#hiddenFilters").addClass("hidden");
$("#lessFiltersButton").addClass("hidden");$("#moreFiltersButton").removeClass("hidden")},openFilters:function(){$("#microhorario-filter").removeClass("hidden");$("#openFiltersButton").addClass("hidden");$("#closeFiltersButton").removeClass("hidden")},closeFilters:function(){$("#microhorario-filter").addClass("hidden");$("#openFiltersButton").removeClass("hidden");$("#closeFiltersButton").addClass("hidden")},initialize:function(){this.template=_.template($("#microhorario-template").html());this.waitingTemplate=
_.template($("#microhorario-waiting-template").html());this.noQueryTemplate=_.template($("#microhorario-noquery-template").html())},changeState:function(a,b){void 0==typeof b&&(b=[]);a==this.queryStatus?(this.closeFilters(),microhorarioClasseslistView.render(b),microhorarioClasseslistView.resizeH(),microhorarioClasseslistView.resizeW()):a==this.noQueryStatus?this.$resultsDiv.html(this.noQueryTemplate({noQueryStr:"No query"})):this.$resultsDiv.html(this.waitingTemplate({waitingImgURL:this.waitingImgURL}))},
fetchStrings:function(){return{noQueryStr:"No query",subjectCodeStr:"Codigo da Disciplina:",subjectNameStr:"Nome da Disciplina:",professorNameStr:"Nome do Professor:",toggleBlocked:"Exibir disciplinas bloqueadas",moreFiltersStr:"More filters",lessFiltersStr:"Less filters",openFiltersStr:"Open filters",closeFiltersStr:"Close filters"}},render:function(){this.$el.html(this.template(this.fetchStrings()));this.$resultsDiv=$("#microhorario-results");microhorarioClasseslistView.setElement(this.$resultsDiv);
...
else{var b=a.target.parent();$($(b).find("li")).each(function(a,b){$(b).removeClass("btn-primary")});$(a.target).addClass("btn-primary")}},resizeW:function(){},resizeH:function(){},sortableConfig:function(a){var b=this;$(a).sortable({connectWith:".selectedSortable",receive:function(a,d){3<$(this).children().length&&$(d.sender).sortable("cancel");0==$(d.sender).children().length&&$(d.sender).remove();if($("#main-selected-div ul").last().children().length){var e=document.createElement("ul");$(e).addClass("selectedSortable");
$("#main-selected-div").append(e);b.sortableConfig(e)}}})},initJS:function(){this.sortableConfig("#main-selected-div ul")},buildRow:function(a){var b=document.createElement("ul");$(b).addClass("selectedSortable");for(var c=0;c<a.length;c++)c?$(b).append('<li class="btn disabled">'+a[c].subcode+"-"+a[c].classcode+"</li>"):$(b).append('<li class="btn btn-primary disabled">'+a[0].subcode+"-"+a[0].classcode+"</li>");return b},buildSelected:function(a){var b=document.createElement("div");a[a.length]=[];
for(var c=0;c<a.length;c++)$(b).append(this.buildRow(a[c]));return $(b).html()},fetchData:function(){return[[{subcode:"ENG1232",classcode:"3VA"},{subcode:"ENG1232",classcode:"3VA"}],[{subcode:"ENG1232",classcode:"3VA"}],[{subcode:"ENG1232",classcode:"3VA"},{subcode:"ENG1232",classcode:"3VA"},{subcode:"ENG1232",classcode:"3VA"}]]},render:function(){var a=this.fetchData();this.$el.html(this.buildSelected(a));this.initJS()}}),selectedView=new SelectedView,MainView=Backbone.View.extend({el:"body",template:"",
rendered:!1,tabs:{faltacursar:{li:"main-faltacursar-li",div:"main-faltacursar-div",str:"",href:"#faltacursar",view:faltacursarView},microhorario:{li:"main-microhorario-li",div:"main-microhorario-div",str:"",href:"#microhorario",view:microhorarioView},selected:{li:"main-selected-li",div:"main-selected-div",str:"",href:"#selected",view:selectedView}},defaultTab:"faltacursar",childrenViews:[faltacursarView,timetableView,selectedView,microhorarioView],timetableDiv:"",sidebarDiv:"",timetableTable:"",container:"",
...
$.each(this.childrenViews,function(a,b){b.resizeH()})},setTimetableWFromSidebarW:function(){var a=$(this.container).width(),b=$(this.sidebarDiv).outerWidth(!0),c=$(this.sidebarDiv).width(),d=$(this.timetableDiv).outerWidth(!0),e=$(this.timetableDiv).width(),f=a-b;$(this.timetableDiv).width(f-(d-e)-1);$(this.timetableDiv).width()<$(this.timetableTable).width()&&(f=a-d,$(this.timetableDiv).width(e),$(this.sidebarDiv).width(f-(b-c)-1))},initJS:function(){var a=0;$(this.tabsNav).find("li").each(function(){a+=
$(this).outerWidth(!0)});a+=1;$(this.sidebarDiv).resizable({handles:"e, w",minWidth:a,containment:"parent",resize:this.setTimetableWFromSidebarW})},setActiveTab:function(a){$("#main-tabs-nav li").removeClass("active");$("#main-tab-panes div").removeClass("active");$("#"+this.tabs[a].li).addClass("active");$("#"+this.tabs[a].div).addClass("active");this.tabs[a].view.resizeW();this.tabs[a].view.resizeH()},fetchStrings:function(){this.tabs.faltacursar.str="Falta Cursar";this.tabs.microhorario.str="Micro Horario";
this.tabs.selected.str="Selecionadas"},renderSubviews:function(){timetableView.setElement("#main-timetable-div");timetableView.render();faltacursarView.setElement("#main-faltacursar-div");faltacursarView.render();microhorarioView.setElement("#main-microhorario-div");microhorarioView.render();selectedView.setElement("#main-selected-div");selectedView.render()},equalMainDivsHeight:function(){var a=$(this.timetableDiv).height();$(this.sidebarDiv).height(a);a-=this.tabsNav.height();faltacursarView.$el.height(a);
microhorarioView.$el.height(a);selectedView.$el.height(a)},render:function(){this.$el.html(this.template({tabs:this.tabs}));this.renderSubviews();this.rendered=!0;this.cache();this.initJS();this.resizeW();this.resizeH()}}),mainView=new MainView,ClasseslistView=Backbone.View.extend({template:"",classesDatatable:"",classesTableHead:"",classesTableBody:"",cache:function(){this.classesTableHead=this.$(".dataTables_scrollHead");this.classesTableBody=this.$(".dataTables_scrollBody")},resizeW:function(){this.classesDatatable.fnAdjustColumnSizing(!1)},
resizeH:"",initialize:function(){this.template=_.template($("#classeslist-template").html());this.resizeH=this.options.resizeH},initJS:function(){this.classesDatatable=this.$el.find("table").dataTable({sDom:this.options.sDom,bPaginate:!1,bScrollCollapse:!0,sScrollY:"200px"})},render:function(a){this.$el.html(this.template(a));this.initJS();this.cache()}}),microhorarioClasseslistView=new ClasseslistView({sDom:"t",resizeH:function(){var a=0;this.$el.siblings(":not(.hidden)").each(function(){a+=$(this).outerHeight()});
var b=this.$el.outerHeight()-this.$el.height(),b=this.$el.parent()-a-b;this.$el.height(b);var c=$(this.classesTableHead).outerHeight();$(this.classesTableBody).height(b-c)}}),faltacursarClasseslistView=new ClasseslistView({sDom:"ft",resizeH:function(){}});function SelectedController(){}var selectedController=new SelectedController,MainRouter=Backbone.Router.extend({routes:{"":"main",":tab":"tabs"}}),mainRouter=new MainRouter;
