...
b,c,d),!0;return!1}}),selectedModel=new SelectedModel,HelperModel=Backbone.Model.extend({defaults:{active:!0,text:"",selector:""},deactivate:function(){$(this.get("selector")).data("tooltip",!1);this.set("active",!1);$.ajax({url:"/api/aviso",type:"DELETE",data:'json=["'+this.id+'"]',dataType:"json",success:function(a){}})}}),HelpersList=Backbone.Collection.extend({model:HelperModel,deactivateFromServer:function(a){for(var b in a)this.get(a[b][serverDictionary.get("CodAviso")]).set("active",!1)}}),
...
0}),faltacursarClasseslistView.render(a),faltacursarClasseslistView.resize()))},clickOnEmenta:function(a){a.stopPropagation()},resize:function(){this.subjectDatatable||(this.subjectDatatable=$("#faltacursar-subject-table").dataTable());this.subjectDatatable.fnAdjustColumnSizing(!1);this.calculateTableScroll()},calculateScrollTop:function(a){a=this.subjectDatatable.fnGetPosition($(a)[0]);var b=0;$(this.subjectTableBody).find("tr").slice(0,a+1).each(function(a){b+=$(this).height()});$(this.subjectTableBody).scrollTop(b)},
...
function SelectedController(){this.runSimulation=function(a){var b=[];if(void 0==a||0>a)a=-1;var c=$("#main-selected-div tbody tr"),d=0,e=0;_.each(c,function(c){for(d>a&&$(c).find('input[type="radio"]').first().attr("checked",!0);"none"!=$(c).find('input[type="radio"]:checked').attr("value");){var g=$(c).find('input[type="radio"]:checked').parent("td").next();if(0!=$(g).find("div").length){var h=$(g).find('input[type="hidden"][name="classCode"]').attr("value"),k=$(g).find('input[type="hidden"][name="subjectCode"]').attr("value"),
m=subjectList.get(k).get("Turmas").get(h),w=subjectList.get(k).get("Creditos"),x=!0,s;for(s in b){m.get("Horarios");var p=subjectList.getClass(b[s].classId),l;if(!(l=k==b[s].subjectCode)){l=p;p=m.get("Horarios").models;l=l.get("Horarios").models;var t=!0,y=void 0;for(y in p){var q=p[y];if(q.get("DiaSemana")){for(var z in l){var n=l[z];if(n.get("DiaSemana")&&q.get("DiaSemana")==n.get("DiaSemana")){var u,r,v;u=q.get("HoraInicial");r=n.get("HoraInicial");v=q.get("HoraFinal");n=n.get("HoraFinal");u>r&&
(r=u,v=n);if(v>r){t=!1;break}}}if(!t)break}}l=!t}if(l){x=!1;break}}if(x&&30>=e+w){g="";for(m=0;m<selectedModel.maxRows;m++)if($(c).hasClass("row"+m)){g=m;break}e+=w;b.push({classId:h,subjectCode:k,cssClass:g});break}}$(g).next().find('input[type="radio"]').attr("checked",!0)}d++});timetableView.render(b);selectedView.changeInfo(e,b.length)};this.sortLines=function(){for(var a=[],b=0;b<selectedModel.maxRows;b++)a[b]=[];var c=$("#main-selected-div tbody tr"),b=0;_.each(c,function(c){c=$(c).find(".class-droppable");
var e=0;_.each(c,function(c){var d=a[b],h=e,k;0==$(c).find("div").length?k=null:(k=$(c).find('input[type="hidden"][name="classCode"]').attr("value"),c=$(c).find('input[type="hidden"][name="subjectCode"]').attr("value"),k={subjectCode:c,classId:k,classCode:subjectList.get(c).get("Turmas").get(k).get("CodigoTurma")});d[h]=k;e++});b++});selectedModel.postAll(a)};this.swapPlaces=function(a,b){var c,d,e,f;d=$("#main-selected-div tbody tr");c=$(d).index($(a).parent());d=$(d).index($(b).parent());e=$($(a).parent().children(".class-droppable")).index(a);
